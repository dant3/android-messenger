...
actual val CoreFirebaseModule: Module = module {
    lifecycleComponent { FirebaseInitializer(context = get()) }
    single { CrashlyticsLogSink() } bind LogSink::class
    single { FirebaseAnalyticsTracker(context = get()) } bind AnalyticsTracker::class
}
